...
if( identical(pr.pos2$SOMAS_ID_round2, snp.data$Somamer) ){
  snp.data.new <- cbind(snp.data,
                          pr.pos2[,c("SOMAS_ID_round2", "EntrezGeneSymbol", "Target",
...
RCircos.Set.Core.Components(cyto.info=cyto.info, chr.exclude,
...
#                        chromStart.1= rep(snpPos,  nrow(prbs)),
#                        chromEnd.1= rep(snpPos,  nrow(prbs))
#                        )
...
RCircos.Gene.Connector.Plot(genomic.data= g.data,
                            track.num=1, side='in')
...
#RCircos.Line.Plot(g.data, data.col=5,
#      track.num=3, side='in')
