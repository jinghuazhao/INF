...
t <- openxlsx::read.xlsx(xlsx, sheet=4, colNames=TRUE, skipEmptyRows=TRUE, cols=c(3,7:16,23,24), rows=c(5,1019:1037))
b1 <- with(t, data.frame(chr=paste0("chr",Chr),start=Pos-1,end=Pos,value1=1))
b2 <- with(t, data.frame(chr=paste0("chr",Chr),start=Region.start,end=Region.end,value2=t["Meta-analysis"]/X13))
circos.par("track.height" = 0.1, cell.padding = c(0, 0, 0, 0))
circos.initializeWithIdeogram()
