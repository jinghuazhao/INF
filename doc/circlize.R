...
b1 <- with(b, data.frame(chr=paste0("chr",Chr),start=Pos-1,end=Pos,value1=1,Target=Target))
b2 <- with(b, data.frame(chr=X.chrom,start=chromStart,end=chromEnd,value2=t["Meta-analysis"]/X14,UniProt=UniProt))
circos.par("track.height" = 0.1, cell.padding = c(0, 0, 0, 0))
circos.initializeWithIdeogram()
