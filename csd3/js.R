...
x$data[[2]] <- list(x=a$x, y=a$y, z=a$log10p, text=as.list(apply(sapply(1:6,fixes,a),1,paste,collapse=" ")),
           type="scatter3d", mode="markers", name="cis")
x$data[[2]]$marker$symbol <- rep('circle',nrow(a))
x$data[[2]]$marker$size <- 3
b <- subset(r[cols],col=="blue")
x$data[[1]] <- list(x=b$x, y=b$y, z=b$log10p, text=as.list(apply(sapply(1:6,fixes,b),1,paste,collapse=" ")),
           type="scatter3d", mode="markers", name="trans")
x$data[[1]]$marker$symbol <- rep('circle',nrow(b))
