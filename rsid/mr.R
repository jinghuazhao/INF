...
d <- lapply(gz, function(x) tryCatch(read.delim(gz,as.is=TRUE)[[1]], error=function(e) NULL))
if (nrow(d)==0) return
library(TwoSampleMR)
...
scatter <- mr_scatter_plot(result, dat)
forest <- mr_forest_plot(single)
funnel <- mr_funnel_plot(single)
leaveoneout <- mr_leaveoneout_plot(loo)
pdf(paste0(prefix,".pdf"))
