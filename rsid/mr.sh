...
function mr_recollect()
{
  ...
  Rscript -e '
    ...
    IVW <- read.delim(file.path(rt,"mr-efo.mr")) %>%
           mutate(fdr=p.adjust(pval,method="fdr")) %>%
           select(prot,id.outcome,b,se,pval,fdr,nsnp,disease)
    ...
    Heterogeneity <- read.delim(file.path(rt,"mr-efo.het")) %>%
                     ...
                     select(prot,id.outcome,method,Q,Q_df,Q_pval,fdr,disease)
    ...
    for (sheet in c("GSMR","IVW","Heterogeneity"))
    {
      ...
      writeData(wb,sheet,"gsmr",xy=c(1,1),headerStyle=createStyle(textDecoration="BOLD",
