...
pr <- Sys.getenv("pr")
z <- read.table(paste0(pr, "-rsid.z"), as.is=TRUE, header=TRUE)
ld <- read.table(paste0(pr, "-rsid.ld.gz"),col.names=with(z,rsid))
snp <- read.table(paste0(pr, "-rsid.snp"), as.is=TRUE, header=TRUE)
config <- read.table(paste0(pr,"-rsid.config"),as.is=TRUE,header=TRUE,nrow=50)
if (file.exists(paste0(pr,"-rsid.cred"))) cred <- read.table(paste0(pr,"-rsid.cred"),as.is=TRUE,header=TRUE)

library(openxlsx)
xlsx <- paste0(pr, "-finemap-rsid.xlsx")
