`query <- function(rsid=INF1_aggr[["INF1_rsid"]],catalogue="eQTL",keep=TRUE)
{
  ...
  f <- file.path(INF,"GTEx",out)
  ips <- subset(merge(INF1_aggr,ps,by="hg19_coordinates"),select=-c(hg19_coordinates,Chr,bp))
...
INF <- Sys.getenv("INF")
if(!exists(file.path(INF,"GTEx"))) dir.create(file.path(INF,"GTEx"))
...
save(eQTL,file=file.path(INF,"GTEx",out))
...
write.table(eQTL_overlap,file=file.path(INF,"GTEx",paste0(prefix,"_eQTL.tsv")),quote=FALSE,row.names=FALSE,sep="\t")
...
tbl[tbl>1] <- 1
write.table(as.data.frame.matrix(tbl),file=file.path(INF,"GTEx",paste0(prefix,"_eQTL_matrix.tsv")),quote=FALSE,sep="\t")
library(pheatmap)
pal <- colorRampPalette(c("white","red"))
col <- pal(3)
library(grid)
png(file.path(INF,"GTEx",paste0(prefix,"_eQTL.png")),res=300,width=18,height=18,units="in")
