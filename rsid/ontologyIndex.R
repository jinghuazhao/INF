...
makeCurrentGwascat(genome="GRCh37")
data(efo.obo.g)
efo.obo.g
...
fang_list <- subset(read.delim(file.path(INF,"files","fang.list.txt")),fullname!="Osteoarthritis")
...
options(width=200)
inf1 <- unique(inf1)
write.table(inf1,file=file.path(INF,"files","fang.efos.txt"),quote=FALSE,row.names=FALSE,sep="\t")
...
save(efo,diseases,fang_disease,isd,efoidname,goidname, file=file.path(INF,"ebi","efo-3.26.0","efo.rda"))
write.table(diseases,file=file.path(INF,"ebi","efo-3.26.0","efo_diseases.csv"),col.names=FALSE,row.names=FALSE,sep=",")
write.table(isd,file=file.path(INF,"ebi","efo-3.26.0","efo_0000540.csv"),col.names=FALSE,row.names=FALSE,sep=",")

library(ontologyPlot)
pdf(file.path(INF,"ebi","efo-3.26.0","efo_0000540.pdf"),height=15,width=15)
...
# adgr
# write.table(as.data.frame(adgr),file=file.path(INF,"files","gwas_autoinfl.tsv"),quote=FALSE,row.names=FALSE,sep="\t")
