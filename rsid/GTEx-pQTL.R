query <- function(rsid=INF1_aggr[["INF1_rsid"]],catalogue="eQTL",keep=TRUE)
{
  ...
  lapply(r,dim)
  ps <- with(r,right_join(snps[c("snp","rsid","hg19_coordinates","a1","a2","consequence","hgnc","snpid")],
                          subset(results,study=="GTEx",select=-c(hg38_coordinates, efo, trait, probe, exp_ensembl,
...
library(dplyr)
library(pQTLtools)
proxies <- "None"; p <- 5e-8; r2 <- 1; build <- 37; prefix <- "cis-pQTL-GTEx-pQTL"; out <- paste0(prefix,".eQTL");
INF <- Sys.getenv("INF")
...
eQTL <- within(subset(ps,hgnc==exp_gene), {gene_snpid <- paste0(hgnc,"-",snpid)}) %>%
              select(hgnc,rsid,hg19_coordinates,a1,a2,consequence,study,pmid,tissue,exp_gene,beta,se,p,gene_snpid)
