...
do
  ...
  awk -vOFS="\t" '{print $2, $3, $4, $5, $6, $7, $8}' > work/${prot}-pQTL.se
done
...
join -j5 <(sort -k5,5 work/MCP.1-pQTL.se) <(sort -k5,5 work/MCP.2-pQTL.se) | \
join -25 - <(sort -k5,5 work/MCP.3-pQTL.se) | \
join -25 - <(sort -k5,5 work/MCP.4-pQTL.se) | \
join -25 - <(sort -k5,5 work/mono-QTL.se) | \
awk -vOFS="\t" '
{
  if($6!=12) $11=-$11
  if($6!=18) $17=-$17
...
awk 'a[$1]++==0' | \
awk 'a[$2]++==0' | \
sort -k3,3n -k4,4n > work/rs12075-se.gassoc
