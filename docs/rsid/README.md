...
2.  Seeing that $\mbox{Var}(\mbox{y})=\mbox{Var}(\mbox{x})(b^2+N\mbox{Var}(b))=\frac{\mbox{Var}(\mbox{x})}{\mbox{Var}(\mbox{b})}(z^2+N)=\frac{2f(1-f)(z^2+N)}{\mbox{Var}(b)}$. When $\mbox{Var}(\mbox{y})=1$, the effect size ($b$) and its standard error ($se$) based on allele frequency ($f$), sample size ($N$) and $z$-statistic ($z$) as in cis eQTLGen data, take forms
