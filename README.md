...
To implement the analysis plan, we started with analysis on INTERVAL as with [INTERVAL.sh](tryggve/INTERVAL.sh) and [cardio.sh](doc/cardio.sh). Implementation has also been done with cis/trans classification ([cis.vs.trans.classification.R](doc/cis.vs.trans.classification.R),[cistrans.sh](doc/cistrans.sh))
