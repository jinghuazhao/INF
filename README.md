...
5. TRYGGVE-specific issues were noted in [tryggve.md](tryggve.md). The `cis.vs.trans.classification`, `circos.cis.vs.trans.plot` (for the circular plot at the top of the page); together with `log10p`, `gc.lambda`, `invnormal`, `METAL_forestplot`, `mhtplot.trunc` functions they are now in [R/gap](https://github.com/jinghuazhao/R/tree/master/gap) repository, as well as gap 1.2.1 at CRAN.
