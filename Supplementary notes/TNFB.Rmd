---
title: "TNFB/LTA GSMR/TwoSampleMR analysis"
date: "`r Sys.Date()`"
toc: false
...
knitr::kable(filter(gsmr,fdr<=0.05) %>% select(gene,outcome,z,nsnp,fdr),caption="Table 1. GSMR results for TNFB (fdr<=0.05)",digits=4)
```
...
knitr::kable(mutate(outcome_dat,disease=gsub("[ |]* id:[a-z]*-[a-z]-[A-Z]*[0-9]*|","",outcome)) %>%
             ...
             setNames(c("SNP","SNPid","A1","b","se","p","N","disease")),caption="Extracted SNP from diseases",digits=3)
dat <- harmonise_data(exposure_dat, outcome_dat, action = 2)
...
with(dat, mr_steiger(p_exp=pval.exposure,p_out=pval.outcome,n_exp=samplesize.exposure,n_out=samplesize.outcome,r_xxo=1,r_yyo=1,r_exp=0))
...
knitr::kable(mutate(outcome_dat,disease=gsub("[ |]* id:[a-z]*-[a-z]-[A-Z]*[0-9]*|","",outcome)) %>%
             ...
             setNames(c("SNP","SNPid","A1","b","se","p","N","disease")),
                      caption="Table 8. SNP information on inconsistent single/IVW MR results",digits=3)
```
